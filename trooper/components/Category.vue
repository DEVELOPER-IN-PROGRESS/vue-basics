<template>
  ...
  <p>{{checkedGenderCategories}}</p>
   <TheFilter @filterItems="filterItems"></TheFilter>
...
export default {
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  setup(props, context) {
    let checkedGenderCategories = ref([]);
    const th = useUiHelpers();
    const uiState = useUiState();
    const { addItem: addItemToCart, isInCart } = useCart();
    const { send: sendNotification } = useUiNotification();
    const { addItem: addItemToWishlist } = useWishlist();
    let { result, search, loading } = useFacet();
    const products = computed(() => facetGetters.getProducts(result.value));
    // const fproducts = computed(() => facetGetters.getProducts(result.value).filter((item)=> item._slug.includes(checkedGenderCategories.value)));
    let cat = JSON.parse(JSON.stringify(checkedGenderCategories.value))
    const fproducts = computed(() => facetGetters.getProducts(result.value));
    // .filter((item)=>cat.some(element => item._slug.includes(element)) )
    
    // .filter((item)=> item._slug.includes('womens'))
    ...
    function filterItems (gender){
      checkedGenderCategories = ref([]);
      gender.forEach(element => {
       checkedGenderCategories.value.push(element);
      
      });
       console.log(checkedGenderCategories.value)
       console.log(fproducts);
       let qwe = ['womens'];
       let asd = fproducts.value.filter(item=> qwe.some(element => item._slug.includes(element)));
       console.log(asd);
       console.log(result)
      //  filter((item)=>cat.some(element => item._slug.includes(element)))
       result.data.products = null;
       asd.forEach(element => {
       result.data.products.push(element);
      });
      //  checkedGenderCategories = JSON.parse(JSON.stringify(gender))
      //  console.log(checkedGenderCategories)
      // gender.map((item)=> console.log(item));
      // products = gender.map(cat=>{ return products.filter((item)=> item._slug.includes(cat))});
    }
  ...
  methods: {
    // asd(gender){
      //Filter using Gender y
      // this.checkedGenderCategories = gender;
      // console.log(this.checkedGenderCategories);
      // var azx = this.checkedGenderCategories.map(cat=>{ return this.products.filter((item)=> item._slug.includes(cat))});
      // console.log(azx)
    // },
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
